...
output "zone_arn" {
  value = var.zone_id != "none" ? var.zone_id : (
      var.vpc_id == "none" ? "arn:aws:route53:::hostedzone/${element(aws_route53_zone.this_public.*.id, 0)}" : "arn:aws:route53:::hostedzone/${element(aws_route53_zone.this_private.*.id, 0)}"
    )
}
